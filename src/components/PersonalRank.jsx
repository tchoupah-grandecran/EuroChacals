...
const PersonalRank = ({ user }) => {
  ...
  useEffect(() => {
    const fetchPersonalRank = async () => {
      try {
        // 1. Récupérer le statut du verrouillage de la session
        ...
        if (userDocSnap.exists()) {
          ...
          if (userData.myPersonalRank) {
            const savedOrderIds = userData.myPersonalRank;
  ...
  const openNotesModal = (country) => {
    if (isVotesLocked) return; // Empêche l'édition des notes si verrouillé
  ...
  return (
    <div style={styles.container}>
      {/* ℹ️ Carte d'information adaptative selon le verrouillage */}
      {isVotesLocked ? (
